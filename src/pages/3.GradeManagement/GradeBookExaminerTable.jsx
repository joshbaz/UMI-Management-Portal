...
const GradeBookExaminerTable = ({ examiners, bookId, refetchBook, book, examinerAssignments }) => {
  ...
  const data = useMemo(() => {
    ...
    return examinerAssignments.map(assignment => ({
  ...
  const columns = useMemo(() => [
    ...
    },
    {
      ...
      header: 'Report',
      cell: (info) => info.getValue() ? 'Submitted' : 'Pending'
    },
    {
      ...
      cell: (info) => (
        <div className="flex gap-2">
          <button
            ...
            onClick={() => navigate(`/grades/book/examiner/${bookId}/${info.row.original.assignmentId}`)}
          >
            View
          </button>
          <button
            className="px-2 py-1 text-xs font-[Inter-Medium] text-white bg-primary-600 rounded hover:bg-primary-700"
            onClick={() => navigate(`/grades/book/update-examiner/${bookId}/${info.row.original.assignmentId}`)}
          >
            Update
          </button>
        </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full border-collapse">
            ...
            {table.getHeaderGroups().map(headerGroup => (
              <tr key={headerGroup.id} className="bg-gray-50 border-y border-gray-200">
                {headerGroup.headers.map(header => (
                  <th 
                    key={header.id}
                    className="px-6 py-3 text-left text-sm font-[Inter-Medium] text-gray-500 capitalize tracking-wider"
          ...
          <tbody>
            {data.length === 0 ? (
              <tr>
                <td colSpan={7} className="px-6 py-4 text-sm font-[Inter-Medium] text-gray-500 text-center">
