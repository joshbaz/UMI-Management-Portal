...
const EditSupervisorUserAccess = ({ formRef, changeSupervisorPasswordMutation, supervisorData }) => {
  ...
  return (
    ...
    onSubmit={(values) => {
      changeSupervisorPasswordMutation.mutate(values);
    }}
  >
    {({ values, setFieldValue, handleChange, errors, touched }) => (
      <Form>
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            disabled={changeSupervisorPasswordMutation.isPending}
            className={`inline-flex items-center px-4 py-2 text-sm font-medium text-white bg-primary-600 rounded-lg hover:bg-primary-700 ${
              changeSupervisorPasswordMutation.isPending
                ? "opacity-50 cursor-not-allowed"
                : ""
            }`}
          >
            <HiOutlineDocumentDuplicate className="w-4 h-4 mr-2" />
            {changeSupervisorPasswordMutation.isPending ? "Saving..." : "Save Details"}
          </button>
        </div>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="text"
                value={supervisorData.facultyType || ''}
                disabled
            ...
            <div>
              ...
              <input
                type="text"
                  value={supervisorData.workEmail || ''}
                disabled
                className="mt-1 block w-full text-gray-500 rounded-md border border-gray-300 bg-gray-100 p-2"
              />
            </div>
...
export default EditSupervisorUserAccess