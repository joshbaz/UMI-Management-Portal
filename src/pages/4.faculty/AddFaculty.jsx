...
import FacultyPersonalInfoForm from "./Forms/FacultyPersonalInfoForm";
import FacultyProfessionalSummary from "./Forms/FacultyProfessionalSummary";
import FacultyUserAccess from "./Forms/FacultyUserAccess";
import { useMutation } from "@tanstack/react-query";
import { createFacultyService } from "@/store/tanstackStore/services/api";
...
const AddFaculty = () => {
  ...
  const createFacultyMutation = useMutation({
    mutationFn: createFacultyService,
    onSuccess: () => {
      // Clear only form-related items from localStorage
      localStorage.removeItem('facultyPersonalInfo');
      localStorage.removeItem('facultyProfessionalSummary');
      localStorage.removeItem('facultyUserAccess');
      localStorage.removeItem('facultyFinalData');
  ...
  const FormDisplay = (step) => {
    switch (step) {
      case 1:
        return <FacultyPersonalInfoForm formRef={formRef} handleNext={handleMoveToStep} />;
      case 2:
        return <FacultyProfessionalSummary formRef={formRef} handleNext={handleMoveToStep} />;
      case 3:
        return <FacultyUserAccess formRef={formRef} handleNext={handleMoveToStep} createFacultyMutation={createFacultyMutation} />;
      default:
        return null;
    }
  };
        ...
        <div className="w-[968] min-h-full mx-8 mb-8 rounded-lg p-6 bg-white shadow-sm overflow-x-hidden">
          ...
          <div className="mt-8 flex justify-end gap-4">
            ...
            {currentStep < STEPS.length ? (
              ...
              >
                {createFacultyMutation.isPending ? 'Saving...' : 'Save'}
              </button>
            )}
          </div>
        </div>
